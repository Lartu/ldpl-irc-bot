...
std::string IRCBOT_SERVER = "";      //195.154.200.232
std::string IRCBOT_PORT = "6667";    //6667
std::string IRCBOT_NICK = "";        //SirLogsalot
std::string IRCBOT_CHANNELS = "";    //#WatchPeopleCode,#WatchCodeTest
std::string IRCBOT_TO;
std::string IRCBOT_MESSAGE;
std::string IRCBOT_CHANNEL;
std::string IRCBOT_FROM;
int socket_desc;
...
void IRCBOT_SEND_MESSAGE(){
    ...
    sprintf(message_packet, "PRIVMSG %s :%s\r\n", (char*)IRCBOT_TO.c_str(), (char*)IRCBOT_MESSAGE.c_str());
...
int IRCBOT_CREATE_BOT() {
    ...
    server.sin_addr.s_addr = inet_addr(IRCBOT_SERVER.c_str());
    server.sin_family = AF_INET;
    server.sin_port = htons(atoi(IRCBOT_PORT.c_str()));

    if (connect(socket_desc, (struct sockaddr *) &server, sizeof(server)) < 0){
        perror(("Could not connect to " + IRCBOT_SERVER + ":" + IRCBOT_PORT).c_str());
        exit(1);
    }

    set_nick(socket_desc, (char*)IRCBOT_NICK.c_str());
    send_user_packet(socket_desc, (char*)IRCBOT_NICK.c_str());
    join_channel(socket_desc, (char*)IRCBOT_CHANNELS.c_str());
